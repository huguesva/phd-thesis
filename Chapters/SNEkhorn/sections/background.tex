...
% Intuitively, entropy quantifies the average quantity of information conveyed by identifying the outcome of a random variable.
% \nc{est ce que ça donne une sens + intuitif ? }.
% 
% For a matrix $\Pb \in \R^{n \times n}_+$, we denote by $\operatorname{H}_{\mathrm{r}}(\Pb) = \left( \operatorname{H}(\Pb_{i:}) \right)_{i}$ the vector of row-wise entropies. 
...
% In a sparse region, large values for the distances lead to most points
% lying in the tail of the Gaussian Markov chain transition probability. Therefore
% this probability is almost uniform and, without correction, no information can
% be recovered about the neighbors of points in this region.
% In \cref{fig:toy_kde} \hug{figure?}, one can get a glimpse of the importance of adaptive bandwidth on a typical DR task.

\textbf{Dimension Reduction with SNE/t-SNE.} One of the main applications of EAs
...
\centering
\includegraphics[width=\linewidth]{figures/fig_coil.pdf}
...
\textbf{Symmetric Entropy-Constrained Optimal Transport.} Entropy-regularized
