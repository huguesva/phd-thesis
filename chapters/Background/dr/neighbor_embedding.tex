...
\begin{align}\tag{NE}\label{eq:ne_attraction_repulsion}
    \min_{\mZ \in \R^{N \times d}} \: - \sum_{ij} [\mA_{\mX}]_{ij} \log [\mA_{\mZ}]_{ij} + \mathcal{L}_{\mathrm{rep}}(\mZ) \:.
\end{align}
...
\begin{align}
    \mathcal{L}_{\mathrm{rep}}(\mZ) = \sum_{i} \log \Big(\sum_j [\mA_{\mZ}]_{ij} \Big)
\end{align}
...
\begin{align}
    \mathcal{L}_{\mathrm{rep}}(\mZ) = \log \Big(\sum_{ij} [\mA_{\mZ}]_{ij} \Big) \:,
\end{align}
...
\begin{align}
    \max_{\theta = \mZ} \mathbb{E}_{(i,j) \sim p} \log q_{\theta} \big( (i,j) \big) \:.
\end{align}
...
Applying the NCE technique to the t-SNE repulsion term $\mathcal{L}_{\mathrm{rep}}(\mZ)$ has led to the development of several methods that have proven effective in practice. For what follwos we introduce the matrix $\widehat{\mA_{\mZ}}$ such that $[\widehat{\mA_{\mZ}}]_{ij} = [\mA_{\mZ}]_{ij} / ([\mA_{\mZ}]_{ij} + 1)$. The first such approach was LargeVis \citep{tang2016visualizing}, which simplifies to the following objective to minimize over the embeddings $\mZ$:
