...
We start by considering the most canonical MRF which is the multivariate Gaussian model. Note that in this case, integrability translates into the precision matrix being full-rank. As we see with the following, the natural extension of our framework to such models leads to a well-established PCA algorithm. In the following, for a continuous variable $\bm{\Theta}_{Z}$, $\mathbb{P}(\bm{\Theta}_{Z} = \cdot)$ denotes its density.
...
We now consider other types of MRFs, characterized by discrete structuring graphs. We follow the same methodology as in \Cref{sec:pca_graph_coupling}. Specifically, we first define an MRF conditioned on the data given a graph, and then construct a prior for the graph that is conjugate with this conditional distribution.

Therefore let us start by defining the distribution of the observations given a graph. The latter takes the form of a pairwise MRF model which as we show is improper (\textit{i.e.}\ not integrable on $\mathbb{R}^{N \times p}$) when shift-invariant kernels are used. We consider a fixed directed graph $\mW \in \mathcal{S}_{W}$ where:
