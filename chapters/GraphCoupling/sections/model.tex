...
\end{restatable}
\todo{make it compatible with pca def}
...
In this work, the dependency structure among rows of the data is governed by a graph. The strength of the connection between two nodes is given by a symmetric\footnote{\ie $k(-\vx) = k(\vx)$ for any $\vx$ in $\R^p$.} function $k: \mathbb{R}^p \to \mathbb{R}_+$. We consider the following pairwise MRF unnormalized density function:
